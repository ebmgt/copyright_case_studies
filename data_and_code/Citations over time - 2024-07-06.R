...
for (df in 1:10) {  # Start from 1 to include the linear model
  ...
  data.summary <- rbind(data.summary, data.frame(df = df, 
       ...
       most_important_knot = most_important_knot, 
       slope_change = most_important_slope_change, 
...
mtext(paste0("\u2020 Topic search terms at PubMed.gov: ",topic, " AND MEDLINE[SB]"), topic, side = 1, line = 6, adj = 0, cex = 0.8)
